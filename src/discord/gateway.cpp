...
namespace backend {
    ...
    gateway::gateway(Bot *const bot, bool compress, encoding enc) :
        Connection(bot), workers(NUM_THREADS), rate_sem{rate_limit} {
            // set the websocket call back and close handlers
            client.set_message_handler(
                [this](const websocket_incoming_message &msg) {
                    this->handle_callback(msg);
                }
            );
            client.set_close_handler(
                [this](
                        const wss_close_status close_status,
    ...
    void gateway::run() {
        ...
        while(bot->status != TERMINATE) {
            try {
                boost::asio::post(workers, [&]{ this->heartbeat(p); });
