...
namespace backend {
    ...
    pplx::task<nlohmann::json> client::identify() {
        ...
        web::http::client::http_client client{ {U(base_uri +  "/gateway/bot")}};
        return client.request(request).then(
            [this](web::http::http_response response) -> nlohmann::json {
                ...
                auto parsed_json = json_dump.empty() ?
                    nlohmann::json{} :
                    nlohmann::json::parse(json_dump);
                return parsed_json;
            }
        );
    }
    ...
    void client::default_headers(http_request &request) {
        ...
        request.headers().add(
            U("Authorization"), utility::conversions::to_string_t("Bot " + bot->token)
        );
        request.headers().add(U("Content-Type"), U("application/json"));
        request.headers().add(
            U("User-Agent"), U("DiscordBot https://github.com/OfficerMuffins/Pearlbot 1.0.0")
        );
    }
}
